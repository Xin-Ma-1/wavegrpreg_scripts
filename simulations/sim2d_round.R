...
for(a in 1:dims[1]){
  for(b in 1:dims[2]){
    if((u[a]-9*pi/80)^2+(v[b]-9*pi/80)^2<=(11*pi/160)^2){
      beta2.0[a,b] = 1
    }
    if((u[a]-19*pi/80)^2+(v[b]-17*pi/80)^2<=(11*pi/160)^2){
      beta2.0[a,b] = 1/2
    }
  }
}
...
for(a in 1:dims[1]){
  for(b in 1:dims[2]){
    if((u[a]-9*pi/80)^2+(v[b]-9*pi/80)^2<=(5*pi/80)^2){
      beta3.0[a,b] = 4/3
    }
    if((u[a]-19*pi/80)^2+(v[b]-17*pi/80)^2<=(5*pi/80)^2){
      beta3.0[a,b] = 4/5
    }
  }
}
...
save(beta.wnet,          pmse.wnet,
     ...
     file=paste0(workingdir,"/results/sim2d_round_error",rat,"_rep",ind,".Rdata"))
