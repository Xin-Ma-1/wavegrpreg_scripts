...
for(a in 1:dims[1]){
  for(b in 1:dims[2]){
    if(u[a]>=(4*pi/80) & u[a]<=(14*pi/80) & v[b]>=(4*pi/80) & v[b]<=(14*pi/80)){beta[[1]][a,b] = 3/5}
    if(u[a]>=(14*pi/80) & u[a]<=(24*pi/80) & v[b]>=(14*pi/80) & v[b]<=(24*pi/80)){beta[[1]][a,b] = 1}
    if(u[a]>=(9*pi/160) & u[a]<=(27*pi/160) & v[b]>=(9*pi/160) & v[b]<=(27*pi/160)){beta[[2]][a,b] = 1}
    if(u[a]>=(29*pi/160) & u[a]<=(47*pi/160) & v[b]>=(29*pi/160) & v[b]<=(47*pi/160)){beta[[2]][a,b] = 1/2}
    if(u[a]>=(5*pi/80) & u[a]<=(13*pi/80) & v[b]>=(5*pi/80) & v[b]<=(13*pi/80)){beta[[3]][a,b] = 4/3}
    if(u[a]>=(15*pi/80) & u[a]<=(23*pi/80) & v[b]>=(15*pi/80) & v[b]<=(23*pi/80)){beta[[3]][a,b] = 4/5}
  }
}
...
save(beta.wnet,          pmse.wnet,
     ...
     file=paste0(workingdir,"/results/sim2d_unknown_square_error",rat,"_rep",ind,".Rdata"))
