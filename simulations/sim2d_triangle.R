...
for(a in 1:dims[1]){
  for(b in 1:dims[2]){
    if(u[a]>=5/40 & u[a]<=19/40 & (u[a]/2+v[b])>=29/80 & (v[b]-u[a]/2)<=19/80){
      beta2.0[a,b] = 1
    }
    if(u[a]>=21/40 & u[a]<=35/40 & (u[a]/2+v[b])>=77/80 & (v[b]-u[a]/2)<=35/80){
      beta2.0[a,b] = 1/2
    }
  }
}
...
for(a in 1:dims[1]){
  for(b in 1:dims[2]){
    if(u[a]>=3/20 & u[a]<=9/20 & (u[a]/2+v[b])>=15/40 & (v[b]-u[a]/2)<=9/40){
      beta3.0[a,b] = 4/3
    }
    if(u[a]>=11/20 & u[a]<=17/20 & (u[a]/2+v[b])>=39/40 & (v[b]-u[a]/2)<=17/40){
      beta3.0[a,b] = 4/5
    }
  }
}
...
save(beta.wnet,          pmse.wnet,
     ...
     file=paste0(workingdir,"/results/sim2d_triangle_error",rat,"_rep",ind,".Rdata"))
